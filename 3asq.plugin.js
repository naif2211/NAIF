...
function cardToSummary(el) {
  ...
  const cover = abs(img?.attr("data-src") || img?.attr("data-lazy-src") || img?.attr("src"));
...
const plugin = {
  ...
  async detail(id) {
    ...
    const cover = abs(firstAttr(root, [".summary_image", ".profile-manga", ".tab-summary .summary_image"], ["data-src", "data-lazy-src", "src"]));
    const altTitle = firstText(root, [".alternative", ".post-content_item.manga_alternative .summary-content", ".post-content_item .summary-content"]);
    const author = firstText(root, [".author-content", ".post-content_item.manga-authors .summary-content", ".post-content_item.manga-author .summary-content"]);
    const status = firstText(root, [".post-content_item.manga-status .summary-content", ".post-content_item.manga_status .summary-content"]);
    const description = firstText(root, [".summary__content", ".description-summary .summary__content", ".description-summary"]);
    const lastChapter = firstText(root, [".wp-manga-chapter a", "li.wp-manga-chapter a"]);
  ...
  async tags() {
    ...
    const out = [];
    const seen = new Set();
    const links = doc.querySelectorAll(".genres-content a, .genres a, .manga-genres a");
    for (const a of links) {
